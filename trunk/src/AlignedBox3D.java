...
public class AlignedBox3D {
	...
	public BoxFace getFace(int faceNumber){
		return boxFace[faceNumber];
	}
	...
	public boolean contains(Point3D p) {
		return !isEmpty && p0.x() <= p.x() && p.x() <= p1.x()
				&& p0.y() <= p.y() && p.y() <= p1.y() && p0.z() <= p.z()
				&& p.z() <= p1.z();
	}

	public Point3D getCorner(int i) {
		return new Point3D(((i & 1) != 0) ? p1.x() : p0.x(),
				((i & 2) != 0) ? p1.y() : p0.y(), ((i & 4) != 0) ? p1.z() : p0
						.z());
	}

	// Return the corner that is furthest along the given direction.
	public Point3D getExtremeCorner(Vector3D v) {
		return new Point3D(v.x() > 0 ? p1.x() : p0.x(), v.y() > 0 ? p1.y() : p0
				.y(), v.z() > 0 ? p1.z() : p0.z());
	}
	...
	public boolean intersects(Ray3D ray, // input
		...
		if (!new Sphere(getCenter(), Point3D.diff(p1, p0).length() / 2)
				.intersects(ray, intersection, true)) {
			return false;
		}
		...
		if (ray.direction.x() != 0) {
			...
			if (distance >= 0
				...
				if (p0.y() <= candidatePoint.y()
						&& candidatePoint.y() <= p1.y()
						&& p0.z() <= candidatePoint.z()
						&& candidatePoint.z() <= p1.z()) {
		...
		if (ray.direction.y() != 0) {
			...
			if (distance >= 0
					&& (!intersectionDetected || candidateDistance < distance)) {
				candidatePoint = ray.point(candidateDistance);
				if (p0.x() <= candidatePoint.x()
						&& candidatePoint.x() <= p1.x()
						&& p0.z() <= candidatePoint.z()
						&& candidatePoint.z() <= p1.z()) {
					distance = candidateDistance;
					intersection.copy(candidatePoint);
